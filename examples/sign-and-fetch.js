...
async function main() {
  ...
  try {
    ...
    const out = await page.evaluate(async (url) => {
      ...
      if (typeof window.__sigCallCount !== "number")
        window.__sigCallCount = 100;
      ...
      try {
        ...
        const lastNat = [...cap3]
          .reverse()
          .find(
            (c) => c.fn === "gnarly_x" && c.args && c.args[3] && c.args[3].v,
          );
    ...
    if (!out || out.error) {
      throw new Error(
        "Page signing failed: " + (out && out.error ? out.error : "unknown"),
      );
    }
